## STA254 Correspondence Analysis and Related Methods
## Michael Greenacre

## Week1                                              
## Distance measures, clustering, multidimensional scaling


# IMPORT DATA INTO R
# Computing Step 0

# Suppose data is in a text file or Excel, with columns names in first row
# and row names in first column
# Copy the file to the clipboard and then type the following instruction in R:

data <- read.table("clipboard")	# store the imported data in a dataframe called 'data'

# if there are no row names in column 1 then you have to 
# tell R that the first row consists of column names
# data <- read.table("clipboard",header=T)

summary(data)     # a summary of the data set

# assign names to environmental variables
x <- data[,6]
y <- data[,7]
z <- data[,8]

# assign name to matrix of species abundance data
abcde <- data[,1:5]


# Computing Step 1
# graphs of environmental variables and species abundances

hist(x)			# histogram of Depth data
plot(x,y)		# scatter plot of Pollution vs Depth
boxplot(x~data[,9])	# box and whisker plot

# graph of standardized values of three environmental variables 
xst <- (x-mean(x))/sd(x)			# standardize x
yst <- (y-mean(y))/sd(y)			# standardize y
zst <- (z-mean(z))/sd(z)			# standardize y

plot(xst,yst)					# plot standardized variables
abline(lm(yst~xst),col="red",lty=2)		# add trend line (linear regression)

# all pairwise scatterplots of xst,yst,zst,
xyz.st<-cbind(xst,yst,zst)    # assign name to matrix of standardized environmental data
pairs(xyz.st)
pairs(xyz.st,upper.panel=panel.smooth)   # add trend lines


# all pairwise scatterplots of abcde
pairs(abcde)
pairs(abcde,upper.panel=panel.smooth)   # add trend lines



# DISTANCES BETWEEN SAMPLES
# Computing Step 2

# calculate Euclidean distances based on environmental data (xst,yst)
Dxyz<- dist(cbind(xst,yst,zst))	# distance matrix based on Depth (xst),Pollution (yst),Temperature (zst)

as.matrix(Dxyz)[1:6,1:6]	# inspect a subset of the distance matrix


# Computing step 3 
# calculate Bray-Curtis distances based on species abundance data (counts abcde)
BC<-matrix(rep(0,900),nrow=30)
for(i in 1:30) {
  for(j in 1:30) {
    BC[i,j]<-100*sum(abs(abcde[i,]-abcde[j,]))/(sum(abcde[i,])+sum(abcde[j,]))
                 }
               }

BC[1:6,1:6]				# inspect a subset of the distance matrix


# Bray-Curtis distances based on fourth root transformation of frequencies
sqrt(sqrt(abcde))
abcdet<-sqrt(sqrt(abcde))
BCt<-matrix(rep(0,900),nrow=30)
for(i in 1:30) {
  for(j in 1:30) {
    BCt[i,j]<-100*sum(abs(abcdet[i,]-abcdet[j,]))/(sum(abcdet[i,])+sum(abcdet[j,]))
                 }
               }

BCt[1:6,1:6]				# inspect a subset of the distance matrix


# Computing step 4
# calculations of row profiles and chi-square distances

apply(abcde,1,sum)              # row sums (total in each sample)
apply(abcde,2,sum)              # column sums (total for each species)
sum(apply(abcde,2,sum))         # grand total
abcde/apply(abcde,1,sum)        # row profiles

rowpro<-abcde/apply(abcde,1,sum) 
avepro<-apply(abcde,2,sum)/sum(abcde)

# calculate chi-square distances
chid<-dist(as.matrix(rowpro)%*%diag(1/sqrt(avepro)) )

# compare Bray-Curtis to chi-square
plot(chid,as.dist(BC))




# CLUSTER ANALYSIS
# Computing step 5  cluster analyses of xy
# clustering of weighted Euclidean distances based on x and y

treeD<-hclust(Dxyz)
plot(treeD)
cutree(treeD,4)

plot(xst,yst,type="n")
text(xst,yst,cutree(treeD,4))		# add labels of cluster affiliation (cutree)

# Computing step 6  cluster analyses
# cluster analysis of Bray-Curtis dissimilarities between sites

treeBC<-hclust(as.dist(BC))
plot(treeBC)
cutree(treeBC,3)

# Computing step 7  cluster analyses
# cluster analysis of chi-square distances between sites

treechi<-hclust(chid)
plot(treechi)
cutree(treechi,3)




# MULTIDIMENSIONAL SCALING
# Computing step 8
# Classical Multidimensional scaling (MDS)
# assuming Bray-Curtis dissimilarities are in BC
# (can be matrix or distance object in this case)

MDS.BC <- cmdscale(BC, eig=T) 	# calculate classical MDS

# see what's in the object

names(MDS.BC)

MDS.BC <- cmdscale(BC, eig=T, k=29)

# (don't worry about warning message about negative eigenvalues)


# Computing step 9
# assuming chi-square distances are in chid

MDS.chi <- cmdscale(chid, eig=T, k=29)


# Computing step 10
# check the eigenvalues and plot the points (dots)

MDS.BC$eig			# eigenvalues from MDS based on Bray-Curtis
MDS.chi$eig			# eigenvalues from MDS based on chi-square distance

plot(MDS.BC$points[,1], MDS.BC$points[,2])

# plot the points (sequence numbers)
plot(MDS.BC$points[,1], MDS.BC$points[,2], type="n")
text(MDS.BC$points[,1], MDS.BC$points[,2])

# plot the points (labels)
plot(MDS.BC$points[,1], MDS.BC$points[,2], type="n")
lab.s<-labels(data)[[1]]
text(MDS.BC$points[,1], MDS.BC$points[,2], labels=lab.s)


# Computing step 11
# Nonmetric Multidimensional scaling (MDS)
library(MASS) 			# load library 'MASS' to access function isoMDS()
nMDS.BC<-isoMDS(BC)		# calculate nonmetric MDS

# plot the points (labels)
plot(nMDS.BC$points[,1], nMDS.BC$points[,2], type="n")
lab.s<-labels(data)[[1]]
text(nMDS.BC$points[,1], nMDS.BC$points[,2], labels=lab.s)
