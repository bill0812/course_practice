...
#tf.InteractiveSession 與 tf.Session 的作用相同，
# 只不過 tf.InteractiveSession 在建立時，會自動將自己設定為預設的 session，
# 這樣可以讓我們少打一些字。
# 這裡的 sub 在呼叫 tf.Tensor.eval 執行時，會自動使用預設的 session（也就是 sess）來執行。
