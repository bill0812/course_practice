...
def open_image_and_filter(filter_name,img_name,out_dir) :
    ...
    img_padded = np.pad(img, ((1,1), (1,1), (0,0)), mode='constant')
...
def image_filter(filter_matrix,img_padded,new_img,out_dir,count):
    ...
    for j in range(512) :
        for k in range(512) :
            for i in range(3):
                new_img[j+1,k+1,i] = np.dot(img_padded[j:j+3,k:k+3,i].flatten() ,filter_matrix.flatten())
                # print(new_img[j+1,k+1,i])
                if new_img[j+1,k+1,i] <= 0 :
                    new_img[j+1,k+1,i] = 0
                elif new_img[j+1,k+1,i] >= 255 :
                    new_img[j+1,k+1,i] = 255
                else :
                    pass
...
def filtered_img_show(new_img,out_dir,count):
    new_img = new_img/255
    cv2.imshow('Lena_Processed_' + str(count+1), new_img)
